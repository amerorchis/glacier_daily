...
def events_today(now = date.today().strftime('%Y-%m-%d')):
    ...
    def process_event(event):
        ...
        start = event["times"][0]["timestart"].replace(' ', '').replace(':00', '').lower().lstrip('0')
        end = event["times"][0]["timeend"].replace(' ', '').replace(':00', '').lower().lstrip('0')
        name = event["title"].replace('Campground', 'CG').replace(' (St. Mary)', '').replace(' (Apgar)', '')
        loc = event["location"].split("(")[0].split(",")[0].split('<br>')[0].replace('Campground', 'CG').replace('Visitor Center', 'VC')
        for deletion in deletions:
            loc = loc.replace(deletion, '')
        link = f'http://www.nps.gov/planyourvisit/event-details.htm?id={event["id"]}'
        return {'sortable': sortable, 'string': f'<li>{start}-{end}: {name}, {loc} <a href="{link}">(link)</a></li>'}

...
class WeatherAlertService:
    ...
    MAX_RETRIES = 10
    RETRY_DELAY = 3
    MAX_ALERTS = 2  # Temporary limit max alerts until we find a way to filter relevant ones
    ...
    def format_html_message(self, alerts: List[WeatherAlert]) -> str:
        ...
        message = f'<p style="font-size:14px; line-height:22px; font-weight:bold; color:#333333; margin:0 0 5px;">' \
    ...
    def fetch_alerts(self) -> List[Dict]:
        ...
        for _ in range(self.MAX_RETRIES):
            response = requests.get(f'{self.BASE_URL}/alerts/active/area/MT', headers=self.HEADERS, timeout=10)
    ...
    def filter_local_alerts(self, alerts: List[Dict]) -> List[Dict]:
        ...
        return [
            alert['properties'] for i, alert in enumerate(alerts)
    ...
    def process_alerts(self, alerts: List[Dict]) -> List[WeatherAlert]:
        ...
        for alert in alerts[:self.MAX_ALERTS]:  # Apply temporary limit
            ...
            if alert_type not in seen_headlines:
                ...
                existing_alert = next(a for a in processed_alerts if a.headline.split(' issued')[0] == alert_type)

...
def test_get_flickr_success(mock_env_vars, mock_flickr_response):
    with patch('image_otd.flickr.FlickrAPI') as MockFlickrAPI, \
         patch('image_otd.flickr.urlretrieve') as mock_urlretrieve:
...
def test_process_image_file_not_found():
    ...
    with pytest.raises(ImageProcessingError, match="Image processing failed: .*No such file or directory"):
...
def test_upload_pic_otd_success():
    with patch('image_otd.image_otd.upload_file') as mock_upload, \
...
def test_resize_full_new_image(sample_image):
    with patch('image_otd.image_otd.retrieve_from_json', return_value=(False, None)), \

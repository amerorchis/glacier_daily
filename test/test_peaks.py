...
def test_peak_selection_with_cached_data():
    ...
    with patch('peak.peak.retrieve_from_json', return_value=(True, list(mock_cache.values()))):
...
def test_upload_peak(mock_env_vars):
    ...
    with patch('peak.sat.upload_file', return_value=('https://example.com/peak.jpg', [])) as upload_file:
        result = upload_peak()

        # Verify upload_file was called with correct parameters
        upload_file.assert_called_once_with('peak', expected_filename, 'email_images/today/peak.jpg')
        assert result == 'https://example.com/peak.jpg'

...
def sample_weather_data():
    ...
    return [
        ("West Glacier", 75, 45, "Sunny"),
        ("Logan Pass", 65, 35, "Partly Cloudy")
    ]
...
def test_weather_image_success(mock_season, mock_upload, mock_font, 
                             mock_draw, mock_open, sample_weather_data):
...
@patch('weather.weather_img.upload_weather')
@patch('weather.weather_img.get_season')
def test_weather_image_long_condition(mock_season, mock_upload, mock_font, 
                                    mock_draw, mock_open):

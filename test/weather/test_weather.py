...
def mock_all_weather_services():
    ...
    with patch('weather.weather.get_forecast') as mock_forecast, \
...
@pytest.mark.parametrize("aqi_value,expected_category", [
    (30, "good"),
    (75, "moderate"),
...
def test_invalid_aqi_handling(mock_all_weather_services):
    ...
    weather = WeatherContent()
    assert not any(cat[1] for cat in WeatherContent.AQI_CATEGORIES if cat[1] in weather.message2)
